...
def create_vocabulary(vocabulary_path, data_path, max_vocabulary_size,
  ...
  """
#  if gfile.Exists(os.getcwd()+data_path):
#      print(data_path," File not found!")
#      with gfile.GFile(os.getcwd()+data_path, mode="rb") as f:
#          for line in f:  
#           line = tf.compat.as_bytes(line)
#           print(line)
      
  
  if not gfile.Exists(os.getcwd()+vocabulary_path):
    ...
    with gfile.GFile(os.getcwd()+data_path, mode="rb") as f:
      ...
      with gfile.GFile(os.getcwd()+vocabulary_path, mode="wb") as vocab_file:
        for w in vocab_list:
...
def initialize_vocabulary(vocabulary_path):
  ...
  """
  if gfile.Exists(os.getcwd()+vocabulary_path):
    rev_vocab = []
    with gfile.GFile(os.getcwd()+vocabulary_path, mode="rb") as f:
...
def data_to_token_ids(data_path, target_path, vocabulary_path,
  ...
  Args:
    ...
    normalize_digits: Boolean; if true, all digits are replaced by 0s.
  """
  if not gfile.Exists(os.getcwd()+target_path):
    print("Tokenizing data in %s" % data_path)
    vocab, _ = initialize_vocabulary(vocabulary_path)
    with gfile.GFile(os.getcwd()+data_path, mode="rb") as data_file:
      with gfile.GFile(os.getcwd()+target_path, mode="w") as tokens_file:
        counter = 0
...
#  to_dev_path = dev_path + ".fr"
  from_train_path = data_dir+'\\train.vi'
  to_train_path = data_dir+'\\train.en'
  from_dev_path = data_dir+'\\dev.vi'
  to_dev_path = data_dir+'\\dev.en'
   
#  from_train_path = os.path.join(data_dir,'train_questions.en')
#  to_train_path = os.path.join(data_dir,'train_answers.en')
#  from_dev_path = os.path.join(data_dir,'test_questions.en')
#  to_dev_path = os.path.join(data_dir,'test_answers.en')
  
  print('Checking file presence..')
  if not Path(os.getcwd()+'\\'+from_train_path).is_file():
      print('Cannot find the file: ', from_train_path)
  if not Path(os.getcwd()+'\\'+to_train_path).is_file():
      print('Cannot find the file: ', to_train_path)
  if not Path(os.getcwd()+'\\'+from_dev_path).is_file():
      print('Cannot find the file: ', from_dev_path)
  if not Path(os.getcwd()+'\\'+to_dev_path).is_file():
      print('Cannot find the file: ', to_dev_path)    
  
  print('finished checking file presence..')
...
if __name__ == "__main__":
    ...
    data_dir='/tmp'
    prepare_wmt_data(data_dir,  max_vocabulary_size,  max_vocabulary_size, tokenizer=None)